...
class StackSAModuleMSG(nn.Cell):
    ...
    def init_weights(self):
        for m in x2ms_adapter.nn_cell.modules(self):
            if isinstance(m, x2ms_nn.Conv2d):
                x2ms_adapter.nn_init.kaiming_normal_(m.weight)
                if m.bias is not None:
                    x2ms_adapter.nn_init.constant_(m.bias, 0)
            if isinstance(m, x2ms_nn.BatchNorm2d):
                x2ms_adapter.nn_init.constant_(m.weight, 1.0)
                x2ms_adapter.nn_init.constant_(m.bias, 0)

    def construct(self, xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, features=None, empty_voxel_set_zeros=True):
        """
        ...
        for k in range(len(self.groupers)):
            ...
            new_features = x2ms_adapter.tensor_api.unsqueeze(x2ms_adapter.tensor_api.permute(new_features, 1, 0, 2), dim=0)  # (1, C, M1 + M2 ..., nsample)
            new_features = self.mlps[k](new_features)  # (1, C, M1 + M2 ..., nsample)

            if self.pool_method == 'max_pool':
                new_features = x2ms_adapter.tensor_api.squeeze(x2ms_adapter.nn_functional.max_pool2d(
                    new_features, kernel_size=[1, x2ms_adapter.tensor_api.x2ms_size(new_features, 3)]
                ), dim=-1)  # (1, C, M1 + M2 ...)
            elif self.pool_method == 'avg_pool':
                new_features = x2ms_adapter.tensor_api.squeeze(x2ms_adapter.nn_functional.avg_pool2d(
                    new_features, kernel_size=[1, x2ms_adapter.tensor_api.x2ms_size(new_features, 3)]
                ), dim=-1)  # (1, C, M1 + M2 ...)
            else:
                raise NotImplementedError
            new_features = x2ms_adapter.tensor_api.permute(x2ms_adapter.tensor_api.squeeze(new_features, dim=0), 1, 0)  # (M1 + M2 ..., C)
            new_features_list.append(new_features)

        new_features = x2ms_adapter.cat(new_features_list, dim=1)  # (M1 + M2 ..., C)

        return new_xyz, new_features


class StackPointnetFPModule(nn.Cell):
    ...
    def construct(self, unknown, unknown_batch_cnt, known, known_batch_cnt, unknown_feats=None, known_feats=None):
        """
        Args:
            unknown: (N1 + N2 ..., 3)
            known: (M1 + M2 ..., 3)
        ...
        norm = x2ms_adapter.x2ms_sum(dist_recip, dim=-1, keepdim=True)
        weight = dist_recip / norm

        interpolated_feats = pointnet2_utils.three_interpolate(known_feats, idx, weight)

        if unknown_feats is not None:
            new_features = x2ms_adapter.cat([interpolated_feats, unknown_feats], dim=1)  # (N1 + N2 ..., C2 + C1)
        else:
            new_features = interpolated_feats
        new_features = x2ms_adapter.tensor_api.permute(new_features, 1, 0)[None, :, :, None]  # (1, C, N1 + N2 ..., 1)
        new_features = self.mlp(new_features)

        new_features = x2ms_adapter.tensor_api.permute(x2ms_adapter.tensor_api.squeeze(x2ms_adapter.tensor_api.squeeze(new_features, dim=0), dim=-1), 1, 0)  # (N1 + N2 ..., C)
        return new_features


class VectorPoolLocalInterpolateModule(nn.Cell):
    ...
    def construct(self, support_xyz, support_features, xyz_batch_cnt, new_xyz, new_xyz_grid_centers, new_xyz_batch_cnt, weights):
        """
        Args:
        ...
        self.num_avg_length_of_neighbor_idxs = max(self.num_avg_length_of_neighbor_idxs, x2ms_adapter.tensor_api.item(num_avg_length_of_neighbor_idxs))

        dist_recip = 1.0 / (dist + 1e-8)
        norm = x2ms_adapter.x2ms_sum(dist_recip, dim=-1, keepdim=True)
        # weight = dist_recip / torch.clamp_min(norm, min=1e-8)
        weight = dist_recip / mindspore.ops.clamp(norm, min=1e-8)

        empty_mask = (x2ms_adapter.tensor_api.view(idx, -1, 3)[:, 0] == -1)
        x2ms_adapter.tensor_api.view(idx, -1, 3)[empty_mask] = 0

        interpolated_feats = pointnet2_utils.three_interpolate(support_features, x2ms_adapter.tensor_api.view(idx, -1, 3), x2ms_adapter.tensor_api.view(weight, -1, 3))
        interpolated_feats = x2ms_adapter.tensor_api.view(interpolated_feats, idx.shape[0], idx.shape[1], -1)  # (M1 + M2 ..., num_total_grids, C)
        if self.use_xyz:
            near_known_xyz = x2ms_adapter.tensor_api.view(support_xyz[x2ms_adapter.tensor_api.long(x2ms_adapter.tensor_api.view(idx, -1, 3))], -1, 3, 3)  # ( (M1 + M2 ...)*num_total_grids, 3)
            local_xyz = x2ms_adapter.tensor_api.view((x2ms_adapter.tensor_api.view(new_xyz_grid_centers, -1, 1, 3) - near_known_xyz), -1, idx.shape[1], 9)
            if self.xyz_encoding_type == 'concat':
                interpolated_feats = x2ms_adapter.cat((interpolated_feats, local_xyz), dim=-1)  # ( M1 + M2 ..., num_total_grids, 9+C)
            else:
                raise NotImplementedError

        new_features = x2ms_adapter.tensor_api.view(interpolated_feats, -1, interpolated_feats.shape[-1])  # ((M1 + M2 ...) * num_total_grids, C)
...
class VectorPoolAggregationModule(nn.Cell):
    def __init__(
        ...
        self.separate_local_aggregation_layer = x2ms_nn.Sequential(
            x2ms_nn.Conv1d(num_c_in, num_c_out, kernel_size=1, groups=self.total_voxels, bias=False),
            x2ms_nn.BatchNorm1d(num_c_out),
            x2ms_nn.ReLU()
        )
        ...
        for cur_num_c in post_mlps:
            post_mlp_list.extend([
                x2ms_nn.Conv1d(c_in, cur_num_c, kernel_size=1, bias=False),
                x2ms_nn.BatchNorm1d(cur_num_c),
                x2ms_nn.ReLU()
            ])
            c_in = cur_num_c
        self.post_mlps = x2ms_nn.Sequential(*post_mlp_list)
        
        self.ReshapeWeight1 = x2ms_nn.Sequential(
            x2ms_nn.Conv1d(cur_num_c, 1, kernel_size=1, groups=1, bias=False),
            x2ms_nn.BatchNorm1d(1),
            x2ms_nn.ReLU()
        )
        self.ReshapeWeight2 = x2ms_nn.Sequential(
            x2ms_nn.Conv1d(1, cur_num_c, kernel_size=1, groups=1, bias=False),
            x2ms_nn.BatchNorm1d(cur_num_c),
            x2ms_nn.ReLU()
        )
    ...
    def init_weights(self):
        for m in x2ms_adapter.nn_cell.modules(self):
            if isinstance(m, x2ms_nn.Conv2d) or isinstance(m, x2ms_nn.Conv1d):
                ...
                if m.bias is not None:
                    x2ms_adapter.nn_init.constant_(m.bias, 0)
            if isinstance(m, x2ms_nn.BatchNorm2d) or isinstance(m, x2ms_nn.BatchNorm1d):
                x2ms_adapter.nn_init.constant_(m.weight, 1.0)
                x2ms_adapter.nn_init.constant_(m.bias, 0)
    ...
    def vector_pool_with_voxel_query(self, xyz, xyz_batch_cnt, features, new_xyz, new_xyz_batch_cnt):
        ...
        self.num_mean_points_per_grid = max(self.num_mean_points_per_grid, x2ms_adapter.tensor_api.item(num_mean_points_per_grid))

        num_new_pts = new_features.shape[0]
        new_local_xyz = x2ms_adapter.tensor_api.view(new_local_xyz, num_new_pts, -1, 3)  # (N, num_voxel, 3)
        new_features = x2ms_adapter.tensor_api.view(new_features, num_new_pts, -1, self.num_reduced_channels)  # (N, num_voxel, C)
        new_features = x2ms_adapter.tensor_api.view(x2ms_adapter.cat((new_local_xyz, new_features), dim=-1), num_new_pts, -1)
    ...
    def get_dense_voxels_by_center(point_centers, max_neighbour_distance, num_voxels):
        ...
        x_grids = x2ms_adapter.arange(-R + R / num_voxels[0], R - R / num_voxels[0] + 1e-5, 2 * R / num_voxels[0], device=device)
        y_grids = x2ms_adapter.arange(-R + R / num_voxels[1], R - R / num_voxels[1] + 1e-5, 2 * R / num_voxels[1], device=device)
        z_grids = x2ms_adapter.arange(-R + R / num_voxels[2], R - R / num_voxels[2] + 1e-5, 2 * R / num_voxels[2], device=device)
        x_offset, y_offset, z_offset = x2ms_adapter.meshgrid(x_grids, y_grids, z_grids)  # shape: [num_x, num_y, num_z]
        xyz_offset = x2ms_adapter.cat((
            x2ms_adapter.tensor_api.view(x2ms_adapter.tensor_api.contiguous(x_offset), -1, 1),
            x2ms_adapter.tensor_api.view(x2ms_adapter.tensor_api.contiguous(y_offset), -1, 1),
            x2ms_adapter.tensor_api.view(x2ms_adapter.tensor_api.contiguous(z_offset), -1, 1)), dim=-1
        )
    ...
    def vector_pool_with_local_interpolate(self, xyz, xyz_batch_cnt, features, new_xyz, new_xyz_batch_cnt, weights):
        ...
        voxel_features = x2ms_adapter.forward(
            self.local_interpolate_module, support_xyz=xyz, support_features=features, xyz_batch_cnt=xyz_batch_cnt,
        ...
        voxel_features = x2ms_adapter.tensor_api.view(x2ms_adapter.tensor_api.contiguous(voxel_features), -1, self.total_voxels * voxel_features.shape[-1])
        return voxel_features

    def construct(self, xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, features, weights, **kwargs):
        """
        :param xyz: (N1 + N2 ..., 3) tensor of the xyz coordinates of the features
        :param xyz_batch_cnt: (batch_size), [N1, N2, ...]
        :param new_xyz: (M1 + M2 ..., 3)
        ...
        features = x2ms_adapter.tensor_api.x2ms_sum(x2ms_adapter.tensor_api.view(features, N, -1, self.num_reduced_channels), dim=1)
        ...
        vector_features = x2ms_adapter.tensor_api.permute(vector_features, 1, 0)[None, :, :]  # (1, num_voxels * C, M1 + M2 ...)
...
#             new_features = new_features * weights_torch
        ...
        new_features = x2ms_adapter.tensor_api.permute(x2ms_adapter.tensor_api.squeeze(new_features, dim=0), 1, 0)
        
        return new_xyz, new_features


class VectorPoolAggregationModuleMSG(nn.Cell):
    def __init__(self, input_channels, config):
        ...
        for cur_num_c in self.model_cfg.MSG_POST_MLPS:
            ...
            c_in = cur_num_c
        self.msg_post_mlps = x2ms_nn.Sequential(*shared_mlps)
    ...
    def construct(self, **kwargs):
        ...
        features = x2ms_adapter.cat(features_list, dim=-1)
        features = x2ms_adapter.cat((cur_xyz, features), dim=-1)
        features = x2ms_adapter.tensor_api.permute(features, 1, 0)[None, :, :]  # (1, C, N)
        new_features = self.msg_post_mlps(features)
        new_features = x2ms_adapter.tensor_api.permute(x2ms_adapter.tensor_api.squeeze(new_features, dim=0), 1, 0)  # (N, C)
