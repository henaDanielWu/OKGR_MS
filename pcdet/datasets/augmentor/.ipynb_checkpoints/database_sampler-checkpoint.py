...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        for x in sampler_cfg.SAMPLE_GROUPS:
            class_name, sample_num = x2ms_adapter.tensor_api.split(x, ':')
    ...
    def filter_by_min_points(self, db_infos, min_gt_points_list):
        for name_num in min_gt_points_list:
            name, min_num = x2ms_adapter.tensor_api.split(name_num, ':')
            min_num = int(min_num)
    ...
    def copy_paste_to_image_kitti(self, data_dict, crop_feat, gt_number, point_idxes=None):
        ...
        if 'depth' in kitti_img_aug_type:
            paste_order = x2ms_adapter.tensor_api.argsort(boxes3d[:,0])
        ...
        for _order in paste_order:
            ...
            if 'cover' in kitti_img_aug_use_type:
                ...
                depth_mask[_box2d[1]:_box2d[3],_box2d[0]:_box2d[2],0] = x2ms_adapter.tensor_api.x2ms_min(corners_lidar[_order,:,0])
                depth_mask[_box2d[1]:_box2d[3],_box2d[0]:_box2d[2],1] = x2ms_adapter.tensor_api.x2ms_max(corners_lidar[_order,:,0])
    ...
    def collect_image_crops_kitti(self, info, data_dict, obj_points, sampled_gt_boxes, sampled_gt_boxes2d, idx):
        ...
        raw_center = x2ms_adapter.tensor_api.x2ms_mean(info['bbox'].reshape(2,2), 0)
        new_box = sampled_gt_boxes2d[idx].astype(np.int)
        new_shape = np.array([new_box[2]-new_box[0], new_box[3]-new_box[1]])
        raw_box = x2ms_np.concatenate([raw_center-new_shape/2, raw_center+new_shape/2]).astype(np.int)
        ...
        if (raw_box[2]-raw_box[0])!=new_shape[0] or (raw_box[3]-raw_box[1])!=new_shape[1]:
            new_center = x2ms_adapter.tensor_api.x2ms_mean(new_box.reshape(2,2), 0)
            new_shape = np.array([raw_box[2]-raw_box[0], raw_box[3]-raw_box[1]])
            new_box = x2ms_np.concatenate([new_center-new_shape/2, new_center+new_shape/2]).astype(np.int)
    ...
    def sample_gt_boxes_2d_kitti(self, data_dict, sampled_boxes, valid_mask):
        ...
        sampled_boxes2d = box_utils.boxes3d_kitti_camera_to_imageboxes(boxes3d_camera, data_dict['calib'],
                                                                        data_dict['images'].shape[:2])
        sampled_boxes2d = x2ms_adapter.Tensor(sampled_boxes2d)
        existed_boxes2d = x2ms_adapter.Tensor(data_dict['gt_boxes2d'])
        iou2d1 = x2ms_adapter.tensor_api.numpy(box_utils.pairwise_iou(sampled_boxes2d, existed_boxes2d))
        iou2d2 = x2ms_adapter.tensor_api.numpy(box_utils.pairwise_iou(sampled_boxes2d, sampled_boxes2d))
        iou2d2[range(sampled_boxes2d.shape[0]), range(sampled_boxes2d.shape[0])] = 0
        iou2d1 = iou2d1 if iou2d1.shape[1] > 0 else iou2d2

        ret_valid_mask = ((x2ms_adapter.tensor_api.x2ms_max(iou2d1, axis=1)<self.img_aug_iou_thresh) &
                         (x2ms_adapter.tensor_api.x2ms_max(iou2d2, axis=1)<self.img_aug_iou_thresh) &
                         (valid_mask))

        sampled_boxes2d = x2ms_adapter.tensor_api.numpy(sampled_boxes2d[ret_valid_mask])
    ...
    def initilize_image_aug_dict(self, data_dict, gt_boxes_mask):
        ...
        if self.img_aug_type is None:
            ...
            gt_number = x2ms_adapter.tensor_api.x2ms_sum(gt_boxes_mask).astype(np.int)
    ...
    def copy_paste_to_image(self, img_aug_gt_dict, data_dict, points):
        if self.img_aug_type == 'kitti':
            obj_points_idx = x2ms_np.concatenate(img_aug_gt_dict['obj_index_list'], axis=0)
            point_idxes = -1 * np.ones(len(points), dtype=np.int)
            point_idxes[:obj_points_idx.shape[0]] = obj_points_idx

            data_dict['gt_boxes2d'] = x2ms_np.concatenate([img_aug_gt_dict['gt_boxes2d'], np.array(img_aug_gt_dict['crop_boxes2d'])], axis=0)
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None, sampled_gt_boxes2d=None):
        ...
        obj_points = x2ms_np.concatenate(obj_points_list, axis=0)
        ...
        points = x2ms_np.concatenate([obj_points[:, :points.shape[-1]], points], axis=0)
        gt_names = x2ms_np.concatenate([gt_names, sampled_gt_names], axis=0)
        gt_boxes = x2ms_np.concatenate([gt_boxes, sampled_gt_boxes], axis=0)
    ...
    def __call__(self, data_dict):
        ...
        for class_name, sample_group in self.sample_groups.items():
            if self.limit_whole_scene:
                num_gt = x2ms_adapter.tensor_api.x2ms_sum(np, class_name == gt_names)
            ...
            if int(sample_group['sample_num']) > 0:
                ...
                valid_mask = ((x2ms_adapter.tensor_api.x2ms_max(iou1, axis=1) + x2ms_adapter.tensor_api.x2ms_max(iou2, axis=1)) == 0)
                ...
                valid_mask = x2ms_adapter.tensor_api.nonzero(valid_mask)[0]
                valid_sampled_dict = [sampled_dict[x] for x in valid_mask]
                valid_sampled_boxes = sampled_boxes[valid_mask]

                existed_boxes = x2ms_np.concatenate((existed_boxes, valid_sampled_boxes[:, :existed_boxes.shape[-1]]), axis=0)
                total_valid_sampled_dict.extend(valid_sampled_dict)

        sampled_gt_boxes = existed_boxes[gt_boxes.shape[0]:, :]

        if total_valid_sampled_dict.__len__() > 0:
            sampled_gt_boxes2d = x2ms_np.concatenate(sampled_gt_boxes2d, axis=0) if len(sampled_gt_boxes2d) > 0 else None
            sampled_mv_height = x2ms_np.concatenate(sampled_mv_height, axis=0) if len(sampled_mv_height) > 0 else None
