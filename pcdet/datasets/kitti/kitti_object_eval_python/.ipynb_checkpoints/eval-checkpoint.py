...
def get_thresholds(scores: np.ndarray, num_gt, num_sample_pts=41):
    x2ms_adapter.tensor_api.sort(scores)
    scores = scores[::-1]
    current_recall = 0
    thresholds = []
    ...
    if compute_fp:
        ...
        if compute_aos:
            ...
            for i in range(delta_idx):
                tmp[i + fp] = (1.0 + x2ms_adapter.tensor_api.cos(np, delta[i])) / 2.0
                # tmp.append((1.0 + np.cos(delta[i])) / 2.0)
            # assert len(tmp) == fp + tp
            # assert len(delta) == tp
            if tp > 0 or fp > 0:
                similarity = x2ms_adapter.tensor_api.x2ms_sum(np, tmp)
            else:
                similarity = -1
...
def calculate_iou_partly(gt_annos, dt_annos, metric, num_parts=50):
    ...
    for num_part in split_parts:
        ...
        if metric == 0:
            gt_boxes = x2ms_np.concatenate([a["bbox"] for a in gt_annos_part], 0)
            dt_boxes = x2ms_np.concatenate([a["bbox"] for a in dt_annos_part], 0)
            overlap_part = image_box_overlap(gt_boxes, dt_boxes)
        elif metric == 1:
            loc = x2ms_np.concatenate(
                [a["location"][:, [0, 2]] for a in gt_annos_part], 0)
            dims = x2ms_np.concatenate(
                [a["dimensions"][:, [0, 2]] for a in gt_annos_part], 0)
            rots = x2ms_np.concatenate([a["rotation_y"] for a in gt_annos_part], 0)
            gt_boxes = x2ms_np.concatenate(
                [loc, dims, rots[..., np.newaxis]], axis=1)
            loc = x2ms_np.concatenate(
                [a["location"][:, [0, 2]] for a in dt_annos_part], 0)
            dims = x2ms_np.concatenate(
                [a["dimensions"][:, [0, 2]] for a in dt_annos_part], 0)
            rots = x2ms_np.concatenate([a["rotation_y"] for a in dt_annos_part], 0)
            dt_boxes = x2ms_np.concatenate(
                [loc, dims, rots[..., np.newaxis]], axis=1)
            overlap_part = bev_box_overlap(gt_boxes, dt_boxes).astype(
                np.float64)
        elif metric == 2:
            loc = x2ms_np.concatenate([a["location"] for a in gt_annos_part], 0)
            dims = x2ms_np.concatenate([a["dimensions"] for a in gt_annos_part], 0)
            rots = x2ms_np.concatenate([a["rotation_y"] for a in gt_annos_part], 0)
            gt_boxes = x2ms_np.concatenate(
                [loc, dims, rots[..., np.newaxis]], axis=1)
            loc = x2ms_np.concatenate([a["location"] for a in dt_annos_part], 0)
            dims = x2ms_np.concatenate([a["dimensions"] for a in dt_annos_part], 0)
            rots = x2ms_np.concatenate([a["rotation_y"] for a in dt_annos_part], 0)
            dt_boxes = x2ms_np.concatenate(
                [loc, dims, rots[..., np.newaxis]], axis=1)
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty):
    ...
    for i in range(len(gt_annos)):
        ...
        gt_datas = x2ms_np.concatenate(
            [gt_annos[i]["bbox"], gt_annos[i]["alpha"][..., np.newaxis]], 1)
        dt_datas = x2ms_np.concatenate([
        ...
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for i in range(len(gt_annos)):
                    ...
                    thresholdss += x2ms_adapter.tensor_api.tolist(thresholds)
                ...
                idx = 0
                for j, num_part in enumerate(split_parts):
                    gt_datas_part = x2ms_np.concatenate(
                        gt_datas_list[idx:idx + num_part], 0)
                    dt_datas_part = x2ms_np.concatenate(
                        dt_datas_list[idx:idx + num_part], 0)
                    dc_datas_part = x2ms_np.concatenate(
                        dontcares[idx:idx + num_part], 0)
                    ignored_dets_part = x2ms_np.concatenate(
                        ignored_dets[idx:idx + num_part], 0)
                    ignored_gts_part = x2ms_np.concatenate(
                ...
                for i in range(len(thresholds)):
                    precision[m, l, k, i] = x2ms_adapter.tensor_api.x2ms_max(
                        np, precision[m, l, k, i:], axis=-1)
                    recall[m, l, k, i] = x2ms_adapter.tensor_api.x2ms_max(np, recall[m, l, k, i:], axis=-1)
                    if compute_aos:
                        aos[m, l, k, i] = x2ms_adapter.tensor_api.x2ms_max(np, aos[m, l, k, i:], axis=-1)
    ret_dict = {
        "recall": recall,
        "precision": precision,
        "orientation": aos,
...
def do_coco_style_eval(gt_annos, dt_annos, current_classes, overlap_ranges,
    ...
    mAP_bbox = x2ms_adapter.tensor_api.x2ms_mean(mAP_bbox, -1)
    mAP_bev = x2ms_adapter.tensor_api.x2ms_mean(mAP_bev, -1)
    mAP_3d = x2ms_adapter.tensor_api.x2ms_mean(mAP_3d, -1)
    if mAP_aos is not None:
        mAP_aos = x2ms_adapter.tensor_api.x2ms_mean(mAP_aos, -1)
