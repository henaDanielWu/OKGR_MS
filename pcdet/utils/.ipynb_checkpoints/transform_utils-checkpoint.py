...
def project_to_image(project, points):
    ...
    points = x2ms_adapter.tensor_api.unsqueeze(points, dim=-1)
    project = x2ms_adapter.tensor_api.unsqueeze(project, dim=1)

    # Transform points to image and get depths
    points_t = project @ points
    points_t = x2ms_adapter.tensor_api.squeeze(points_t, dim=-1)
...
def normalize_coords(coords, shape):
    ...
    min_n = -1
    max_n = 1
    shape = x2ms_adapter.flip(shape, dims=[0])  # Reverse ordering of shape
...
def bin_depths(depth_map, mode, depth_min, depth_max, num_bins, target=False):
    ...
    if mode == "UD":
        ...
        indices = -0.5 + 0.5 * x2ms_adapter.sqrt(1 + 8 * (depth_map - depth_min) / bin_size)
    elif mode == "SID":
        indices = num_bins * (x2ms_adapter.log(1 + depth_map) - x2ms_adapter.tensor_api.log(math, 1 + depth_min)) / \
            (x2ms_adapter.tensor_api.log(math, 1 + depth_max) - x2ms_adapter.tensor_api.log(math, 1 + depth_min))
    else:
        raise NotImplementedError

    if target:
        # Remove indicies outside of bounds
        mask = (indices < 0) | (indices > num_bins) | (~x2ms_adapter.isfinite(indices))
        indices[mask] = num_bins

        # Convert to integer
        indices = x2ms_adapter.tensor_api.x2ms_type(indices, mindspore.int64)
    return indices
