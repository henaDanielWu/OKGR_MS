...
plydata_car = PlyData.read("./target_dir/car_2048.ply")
data_car = plydata_car.elements[0].data
target_car = pd.DataFrame(data_car)
plydata_pede = PlyData.read("./target_dir/pedestrian_512.ply")
data_pede = plydata_pede.elements[0].data
target_pede = pd.DataFrame(data_pede)
plydata_cycl = PlyData.read("./target_dir/cyclist_512.ply")
...
def chamfer(p1, p2):
    ...
    return x2ms_adapter.x2ms_mean(d1) + x2ms_adapter.x2ms_mean(d2)

def chamfer_sqrt(p1, p2):
    d1, d2, _, _ = chamfer_dist(p1, p2)
    d1 = x2ms_adapter.x2ms_mean(x2ms_adapter.sqrt(d1))
    d2 = x2ms_adapter.x2ms_mean(x2ms_adapter.sqrt(d2))
    return (d1 + d2) / 2
...
def bilinear_interpolate_torch(im, x, y):
    ...
    """
    x0 = x2ms_adapter.tensor_api.long(x2ms_adapter.floor(x))
    x1 = x0 + 1

    y0 = x2ms_adapter.tensor_api.long(x2ms_adapter.floor(y))
    y1 = y0 + 1

    x0 = x2ms_adapter.clamp(x0, 0, im.shape[1] - 1)
    x1 = x2ms_adapter.clamp(x1, 0, im.shape[1] - 1)
    y0 = x2ms_adapter.clamp(y0, 0, im.shape[0] - 1)
    y1 = x2ms_adapter.clamp(y1, 0, im.shape[0] - 1)
    ...
    wa = (x2ms_adapter.tensor_api.type_as(x1, x) - x) * (x2ms_adapter.tensor_api.type_as(y1, y) - y)
    wb = (x2ms_adapter.tensor_api.type_as(x1, x) - x) * (y - x2ms_adapter.tensor_api.type_as(y0, y))
    wc = (x - x2ms_adapter.tensor_api.type_as(x0, x)) * (x2ms_adapter.tensor_api.type_as(y1, y) - y)
    wd = (x - x2ms_adapter.tensor_api.type_as(x0, x)) * (y - x2ms_adapter.tensor_api.type_as(y0, y))
    ans = x2ms_adapter.t((x2ms_adapter.t(Ia) * wa)) + x2ms_adapter.t(x2ms_adapter.t(Ib) * wb) + x2ms_adapter.t(x2ms_adapter.t(Ic) * wc) + x2ms_adapter.t(x2ms_adapter.t(Id) * wd)
    return ans
...
def sample_points_with_roi(rois, points, points_xyzr, sample_radius_with_roi, num_max_points_of_part=200000):
    ...
    if points.shape[0] < num_max_points_of_part:
        ...
        min_dis, min_dis_roi_idx = distance.min(axis=-1,return_indices=True)
        ...
        point_mask = ops.cat(point_mask_list, axis=0)
...
def sector_fps(points, points_xyzr, num_sampled_points, num_sectors):
    ...
    point_angles = x2ms_adapter.atan2(points[:, 1], points[:, 0]) + np.pi
    ...
    for k in range(num_sectors):
        ...
        cur_num_points = mask.sum().asnumpy().item()
        if cur_num_points > 0:
    ...
    xyz = ops.cat(xyz_points_list, axis=0)
    xyzr = ops.cat(xyzr_points_list, axis=0)
    xyz_batch_cnt = Tensor(xyz_batch_cnt, dtype=mindspore.int32)
    sampled_points_batch_cnt = Tensor(num_sampled_points_list, dtype=mindspore.int32)

    sampled_pt_idxs = pointnet2_stack_utils.stack_farthest_point_sample(
        xyz, xyz_batch_cnt, sampled_points_batch_cnt
    ).long()
...
def in_convex_polyhedron(points_set, test_points):
    ...
    ori_edge_index = x2ms_adapter.tensor_api.sort(np, x2ms_adapter.tensor_api.unique(np, ori_edge_index))
    for i in range(test_points.shape[0]):
        new_set = x2ms_np.concatenate((points_set, test_points[i, np.newaxis]), axis=0)
        new_edge_index = conv_hull(new_set)
        new_edge_index = x2ms_adapter.tensor_api.sort(np, x2ms_adapter.tensor_api.unique(np, new_edge_index))
        bol[i] = (x2ms_adapter.tensor_api.tolist(new_edge_index) == x2ms_adapter.tensor_api.tolist(ori_edge_index))
    return bol
...
def load_data_txt(path):
    file = open(path, 'r')
    data = x2ms_adapter.tensor_api.split(file.read(), '\n')
...
def _data_trans(data):
    lst = []
    for num in data:
        num_list = x2ms_adapter.tensor_api.split(num)
...
def compute_3d_box_cam2(h, w, l, x, y, z, yaw):
    # 计算旋转矩阵
    R = np.array([[x2ms_adapter.tensor_api.cos(np, yaw), 0, x2ms_adapter.tensor_api.sin(np, yaw)], [0, 1, 0], [-x2ms_adapter.tensor_api.sin(np, yaw), 0, x2ms_adapter.tensor_api.cos(np, yaw)]])    # 8个顶点的xyz
...
def normalization_x(x, type='Torch'):
    if type == 'Torch':
        min_x = x2ms_adapter.x2ms_min(x)
        max_x = x2ms_adapter.x2ms_max(x)
        norm_x = (x - min_x) / (max_x - min_x + 1e-4)
    elif type == 'Numpy':
        min_x = x2ms_adapter.tensor_api.x2ms_min(np, x)
        max_x = x2ms_adapter.tensor_api.x2ms_max(np, x)
        norm_x = (x - min_x) / (max_x - min_x + 1e-4)
    else:
        min_x = x2ms_adapter.x2ms_min(x)
        max_x = x2ms_adapter.x2ms_max(x)
...
def SelectClosedProposal(rois_b, rois_labels_b, rois_scores_selected):
    ...
    rois_b_center, rois_labels_center, rois_scores_selected_center = x2ms_adapter.tensor_api.numpy(rois_b), x2ms_adapter.tensor_api.numpy(rois_labels_b), x2ms_adapter.tensor_api.numpy(rois_scores_selected)
    while rois_b_center.shape[0] != 0:
        ...
        if rois_b_center.shape[0] > -1:
            ...
            for j in range(0, rois_b_center_part2.shape[0]):
                if np.absolute(x2ms_adapter.tensor_api.norm(np.linalg, rois_b_center_part1[:3] - rois_b_center_part2[j][:3])) < 0.6:
...
def SphereDistance(point1, point2, C):
    ...
    min_dis, min_dis_idx = x2ms_adapter.tensor_api.x2ms_min(distance, dim=-1)
...
def pc_normalize(pc):
    ...
    centroid = x2ms_np.mean(pc, axis=0)
    pc = pc - centroid
    m = x2ms_adapter.tensor_api.x2ms_max(np, x2ms_adapter.tensor_api.sqrt(np, x2ms_adapter.tensor_api.x2ms_sum(np, pc ** 2, axis=1)))
    # 对点云进行缩放
    pc = pc / m
    return pc

def pc_denormalize(pc_nor, pc_raw):
    centroid = x2ms_np.mean(pc_raw, axis=0)
    pc_raw = pc_raw - centroid
    m = x2ms_adapter.tensor_api.x2ms_max(np, x2ms_adapter.tensor_api.sqrt(np, x2ms_adapter.tensor_api.x2ms_sum(np, pc_raw ** 2, axis=1)))
    pc_denor = pc_nor * m + centroid
    return pc_denor

class VoxelSetAbstraction(nn.Cell):
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
        ...
        self.vsa_point_feature_fusion = nn.SequentialCell(
            nn.Dense(c_in, self.model_cfg.NUM_OUTPUT_FEATURES, has_bias=False),
    ...
    def interpolate_from_bev_features(self, keypoints, bev_features, batch_size, bev_stride, weights=None):
        ...
        point_bev_features = mindspore.ops.cat(point_bev_features_list, axis=0)  # (N1 + N2 + ..., C)
    ...
    def get_sampled_points(self, batch_dict):
        ...
        for bs_idx in range(batch_size):
            ...
            keypoints_xyzr=None
            if self.model_cfg.SAMPLE_METHOD == 'FPS':#不进这里
                cur_pt_idxs = pointnet2_stack_utils.farthest_point_sample(
                    x2ms_adapter.tensor_api.contiguous(sampled_points[:, :, 0:3]), self.model_cfg.NUM_KEYPOINTS
                ).long()

                if sampled_points.shape[1] < self.model_cfg.NUM_KEYPOINTS:
                    times = int(self.model_cfg.NUM_KEYPOINTS / sampled_points.shape[1]) + 1
                    non_empty = cur_pt_idxs[0, :sampled_points.shape[1]]
                    cur_pt_idxs[0] =non_empty.repeat(tuple(times))[:self.model_cfg.NUM_KEYPOINTS]
                ...
                keypoints = ops.cat((bs_idxs[:, None], cur_keypoints), axis=1)
                keypoints_xyzr = ops.cat((bs_idxs_xyzr[:, None], cur_keypoints_xyzr), axis=1)
            else:
                raise NotImplementedError
            keypoints_list.append(keypoints)
            keypoints_xyzr_list.append(x2ms_adapter.tensor_api.detach(keypoints_xyzr))

        keypoints = ops.cat(keypoints_list, axis=0)  # (B, M, 3) or (N1 + N2 + ..., 4)
        if len(keypoints.shape) == 3:
            batch_idx = ops.arange(batch_size).view(-1, 1).repeat((1, keypoints.shape[1])).view(-1, 1)
            keypoints = ops.cat((batch_idx.float(), keypoints.view(-1, 3)), axis=1)
            # keypoints_xyzr = x2ms_adapter.cat((x2ms_adapter.tensor_api.x2ms_float(batch_idx), x2ms_adapter.tensor_api.view(keypoints_xyzr, -1, 3)), dim=1)
    ...
    def aggregate_keypoint_features_from_one_source(
        ...
        if filter_neighbors_with_roi: # PV RCNN++
            point_features = x2ms_adapter.cat((xyz, xyz_features), dim=-1) if xyz_features is not None else xyz
            point_features_list = []
            if potential_occluded_points_list != None and weights == None:
                for bs_idx in range(batch_size):
                    ...
                    if potential_occluded_points_list[bs_idx] != [0]:
                        point_features_new = x2ms_adapter.cat((point_features[bs_mask][valid_mask], potential_occluded_points_list[bs_idx][:, 1:]))
                        point_features_list.append(point_features_new)
                    else:
                        point_features_list.append(point_features[bs_mask][valid_mask])
                    xyz_batch_cnt[bs_idx] = x2ms_adapter.tensor_api.x2ms_sum(valid_mask)
            else:
                for bs_idx in range(batch_size):
                    bs_mask = (xyz_bs_idxs == bs_idx)
                    points_xyzr = None
            ...
            valid_point_features = x2ms_adapter.cat(point_features_list, dim=0)
            xyz = valid_point_features[:, 0:3]
            xyz_features = valid_point_features[:, 3:] if xyz_features is not None else None
        else: # PV RCNN
            for bs_idx in range(batch_size):
                xyz_batch_cnt[bs_idx] = x2ms_adapter.tensor_api.x2ms_sum((xyz_bs_idxs == bs_idx))
        pooled_points, pooled_features = aggregate_func(
            xyz=x2ms_adapter.tensor_api.contiguous(xyz),
            xyz_batch_cnt=xyz_batch_cnt,
            new_xyz=new_xyz,
            new_xyz_batch_cnt=new_xyz_batch_cnt,
            features=x2ms_adapter.tensor_api.contiguous(xyz_features),
            weights=weights,
        )
    ...
    def construct(self, batch_dict):
        ...
        #1.18注释因为batch_dict['roi_targets_dict']中没东西，前面的一层数值出错了
        # if self.training:
        ...
        #     roi_labels = x2ms_adapter.tensor_api.detach(roi_targets_dict['roi_labels']) # [bs, 128]
        #     roi_scores = x2ms_adapter.tensor_api.detach(roi_targets_dict['roi_scores']) # [bs, 128]
        #     rois = x2ms_adapter.tensor_api.detach(roi_targets_dict['rois'])
        #     # gt_of_rois = x2ms_adapter.tensor_api.detach(roi_targets_dict['gt_of_rois_src'])
        #     # print(roi_targets_dict['roi_scores'].shape, roi_targets_dict['roi_labels'].shape, roi_targets_dict['rcnn_cls_labels'])
        #     # print('\n')
        # else:
        roi_scores = x2ms_adapter.tensor_api.detach(batch_dict['roi_scores'])
        roi_labels = x2ms_adapter.tensor_api.detach(batch_dict['roi_labels'])
        rois = x2ms_adapter.tensor_api.detach(batch_dict['rois'])
        ...
        # if epoch_index > 80000:
            ...
            for b in range(batch_dict['batch_size']):
                ...
                roi_scores_sorted, idx_roi_scores = x2ms_adapter.sort(roi_scores[b], descending=True)
                # # score 归一化
                min_a = x2ms_adapter.x2ms_min(roi_scores_sorted)
                max_a = x2ms_adapter.x2ms_max(roi_scores_sorted)
                ...
                rois_b_new_torch = x2ms_adapter.x2ms_tensor(np.array(rois_b_new))
                ...
                for i in range(roi_local_label[-1] + 1):
                    if static_roi_local_label[i] == 1: # 只有一个
                        ...
                        rois_b_mean = x2ms_np.mean(rois_b_tmp, axis=0)
                        rois_scores_mean = x2ms_np.mean(rois_scores_tmp, axis=0)
                        rois_scores_max = x2ms_adapter.tensor_api.x2ms_max(np, rois_labels_temp)
                ...
                rois_b_selected_torch = x2ms_adapter.from_numpy(np.array(rois_b_selected))
                keypoints_b_selected = keypoints_b[:, 1:4]
                ...
                for i in range(roi_local_label[-1] + 1): # 以roi_local_label最后一个数 -- 去除false postive的roi的个数, 循环每一个roi
                    ...
                    if roi_label == 2 or roi_label == 3: # if roi_dz > np.max([roi_dx, roi_dy]): # 竖直圆柱
                        cylinder_r = x2ms_adapter.tensor_api.sqrt(np, roi_dx ** 2 + roi_dy ** 2) / 2
                    else: # 水平圆柱
                        if roi_dx < roi_dy:
                            cylinder_r = x2ms_adapter.tensor_api.sqrt(np, roi_dx ** 2 + roi_dz ** 2) / 2
                        else:
                            cylinder_r = x2ms_adapter.tensor_api.sqrt(np, roi_dy ** 2 + roi_dz ** 2) / 2
                    ...
                    for p in range(len(false_positive_point_mask_list)): # for all false positives
                        for q in range(len(false_positive_point_mask_list[p])): # for all points in one false positive
                            ...
                            if (false_positive_point_mask_list[p][q] in fg_of_rois_b_selected_index and random_mirror <= 1) or (false_positive_point_mask_list[p][q] not in fg_of_rois_b_selected_index and random_mirror <= 1):
                                ...
                                point_x_new, point_y_new,point_r_new=0,0,0
                                # if point_r > 0.2:
                                ...
                                # occluded_points_one_box.append(x2ms_adapter.from_numpy(np.array([[point_n, point_x_new, point_y_new, point_z, point_r_new]])))
                                occluded_points_one_box.append(x2ms_adapter.from_numpy(np.array([[point_n.asnumpy(), point_x_new.asnumpy(), point_y_new.asnumpy(), point_z.asnumpy(), point_r_new.asnumpy()]])))
                    ...
                    if len(occluded_points_one_box) != 0 and len(occluded_points_one_box) != 1:
                        ...
                        whole_object_kp = ops.cat((ops.cat(occluded_points_one_box), fg_of_rois_b_selected))[:, 1:]
                        bs_whole_object_kp = ops.cat((ops.cat(occluded_points_one_box), fg_of_rois_b_selected))[:, 0]
                        f_whole_object_kp = ops.cat((ops.cat(occluded_points_one_box), fg_of_rois_b_selected))[:, 4]
                        ...
                        whole_object_kp = single_rotate_points_along_z(x2ms_adapter.tensor_api.numpy(whole_object_kp), -roi_angle)
                        # whole_object_kp_nor = pc_normalize(whole_object_kp)
                        points = x2ms_adapter.tensor_api.unsqueeze(x2ms_adapter.from_numpy(whole_object_kp.reshape(-1, 3)[:, 0:3]), 0)
                        ...
                        whole_object_kp1 = single_rotate_points_along_z(x2ms_adapter.tensor_api.numpy(x2ms_adapter.tensor_api.detach(x2ms_adapter.tensor_api.squeeze(points_completion[-1], 0))), roi_angle)
                        ...
                        whole_object_kp1 = x2ms_adapter.from_numpy(whole_object_kp1)
                        ...
                        for o0 in range(occ.shape[0]):
                            ...
                            if roi_label == 2 or roi_label == 3:  # if roi_dz > np.max([roi_dx, roi_dy]):
                                # 到proposal竖直中心线的距离
                                l1 = x2ms_adapter.sqrt((roi_x_c - point_x_new) ** 2 + (roi_y_c - point_y_new) ** 2)
                                distance2points_score_one_box.append(l1)
                                if l1 < x2ms_adapter.x2ms_tensor(cylinder_r):
                                    if_in_cylinder_mask.append(True)
                                else:
                                    if_in_cylinder_mask.append(False)
                            else:
                                # 到proposal水平中心线距离
                                l2 = x2ms_adapter.sqrt((roi_x_c - point_x_new) ** 2 + (roi_y_c - point_y_new) ** 2) * x2ms_adapter.x2ms_abs(x2ms_adapter.sin(x2ms_adapter.x2ms_tensor(roi_angle)))
                                distance2points_score_one_box.append(l2)
                                if l2 < x2ms_adapter.x2ms_tensor(cylinder_r):
                                    if_in_cylinder_mask.append(True)
                                else:
                                    if_in_cylinder_mask.append(False)
                        whole_object_kp_one_object = x2ms_adapter.cat((x2ms_adapter.cat((x2ms_adapter.tensor_api.unsqueeze(bs_whole_object_kp, 1), whole_object_kp1), 1), x2ms_adapter.tensor_api.unsqueeze(f_whole_object_kp, 1)), 1)
                        ...
                        weights_fg_points_list.append(x2ms_adapter.ones(1, fg.shape[0]))
                        if len([n for n in if_in_cylinder_mask if n == True]) != 0:
                            kde = KernelDensity(kernel='gaussian', bandwidth=0.3).fit(x2ms_adapter.tensor_api.numpy(occ)[if_in_cylinder_mask])
                            log_density = kde.score_samples(x2ms_adapter.tensor_api.numpy(occ)[if_in_cylinder_mask])
                            density_true_recips = 10 ** (log_density)
                            density_true = normalization_x(density_true_recips, type='Numpy')
                            density = x2ms_np.mean(density_true) * np.ones_like([np.array(if_in_cylinder_mask)])
                            ...
                            # 计算权重
                            temp=[]
                            for i in distance2points_score_one_box:
                                temp.append(i.asnumpy())
                            proposal2points_score_one_box_torch, distance2points_score_one_box_torch = x2ms_adapter.from_numpy(np.array(proposal2points_score_one_box)), x2ms_adapter.from_numpy(np.array(temp))
                            ...
                            weight_matrix = distance2points_score_one_box_torch.reshape(-1, 1) * 0.4 + x2ms_adapter.tensor_api.x2ms_float(x2ms_adapter.from_numpy(density)).reshape(-1, 1) * 0.6  # torchsize(n, 1)
                            weights_fg_points_list.append(x2ms_adapter.tensor_api.unsqueeze(x2ms_adapter.tensor_api.squeeze(weight_matrix, 1), 0))
                ...
                if fg_points_selected_new != []:
                    fg_points_selected_new_torch = x2ms_adapter.cat(fg_points_selected_new)
                    weights_fg_points_torch = x2ms_adapter.cat((weights_fg_points_list), 1)
                    keypoints_b_new = x2ms_adapter.cat((fg_points_selected_new_torch[:, :4], bg_points_selected[:, :4]))
                    weights_all_points_torch = x2ms_adapter.cat((weights_fg_points_torch, x2ms_adapter.ones(1, bg_points_selected.shape[0])), 1)
                ...
                if len(occluded_points_list) != 0:
                    potential_occluded_points_list.append(x2ms_adapter.cat(occluded_points_list))
                else:
            ...
            if keypoints_list_new != []:
                keypoints = x2ms_adapter.cat(keypoints_list_new)
                batch_dict['weights_all_points_all_batch'] = weights_all_points_all_batch
                completion_dict['points_completion_list'] = points_completion_list
                completion_dict['deltas_list'] = deltas_list
                completion_dict['dir_list'] = dir_list
                completion_dict['roi_label_list'] = roi_label_list
                self.completion_dict.update(completion_dict)
        ...
        new_xyz = x2ms_adapter.tensor_api.contiguous(keypoints[:, 1:4])
        new_xyz_batch_cnt = x2ms_adapter.tensor_api.x2ms_int(x2ms_adapter.tensor_api.new_zeros(new_xyz, batch_size))
        for k in range(batch_size):
            new_xyz_batch_cnt[k] = x2ms_adapter.tensor_api.x2ms_sum((keypoints[:, 0] == k))
        ...
        if 'raw_points' in self.model_cfg.FEATURES_SOURCE:
            ...
            pooled_features = self.aggregate_keypoint_features_from_one_source(
                ...
                xyz=raw_points[:, 1:4],
                xyz_features=x2ms_adapter.tensor_api.contiguous(raw_points[:, 4:]) if raw_points.shape[1] > 4 else None,
        ...
        for k, src_name in enumerate(self.SA_layer_names):
            cur_coords = mindspore.Tensor(batch_dict['multi_scale_3d_features'][src_name].indices.detach().numpy())
            cur_features = mindspore.Tensor(batch_dict['multi_scale_3d_features'][src_name].features.detach().numpy())
            ...
            pooled_features = self.aggregate_keypoint_features_from_one_source(
                ...
                xyz=x2ms_adapter.tensor_api.contiguous(xyz), xyz_features=cur_features, xyz_bs_idxs=cur_coords[:, 0],
        ...
        point_features = x2ms_adapter.cat(point_features_list, dim=-1)

        batch_dict['point_features_before_fusion'] = x2ms_adapter.tensor_api.view(point_features, -1, point_features.shape[-1])
        point_features = self.vsa_point_feature_fusion(x2ms_adapter.tensor_api.view(point_features, -1, point_features.shape[-1]))
    ...
    def get_loss(self, tb_dict=None):
        ...
        for i in range(len(deltas_list)):
            ...
            gt = x2ms_adapter.as_tensor(x2ms_adapter.tensor_api.unsqueeze(x2ms_adapter.from_numpy(gt.to_numpy()), 0), dtype=mindspore.float32, device=device)
            ...
            delta_losses = []
            for delta in deltas:
                delta_losses.append(x2ms_adapter.x2ms_sum(delta ** 2))
            loss_pmd = x2ms_adapter.x2ms_sum(x2ms_adapter.stack(delta_losses)) / 2
