...
class PVRCNNPlusPlus(nn.Cell):
        ...
        if cur_gt.shape[0] > 0:
            ...
            for cur_thresh in thresh_list:
                if iou3d_rcnn.shape[0] == 0:
                    ...
                    rcnn_recalled = x2ms_adapter.tensor_api.item(x2ms_adapter.tensor_api.x2ms_sum(
                        (x2ms_adapter.tensor_api.x2ms_max(iou3d_rcnn, dim=0)[0] > cur_thresh)))
                    recall_dict['rcnn_%s' % str(cur_thresh)] += rcnn_recalled
                if rois is not None:
                    roi_recalled = x2ms_adapter.tensor_api.item(x2ms_adapter.tensor_api.x2ms_sum(
                        (x2ms_adapter.tensor_api.x2ms_max(iou3d_roi, dim=0)[0] > cur_thresh)))
    ...
    def load_params_from_file(self, filename, logger, to_cpu=False, pre_trained_path=None):
        ...
        if not pre_trained_path is None:
            pretrain_checkpoint = x2ms_adapter.load(pre_trained_path, map_location=loc_type)
