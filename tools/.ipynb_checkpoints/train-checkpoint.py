...
def main():
    ...
    if args.ckpt is not None:
        ...
        if len(ckpt_list) > 0:
            x2ms_adapter.tensor_api.sort(ckpt_list, key=os.path.getmtime)
    ...
    x2ms_adapter.x2ms_train(model)  # before wrap to DistributedDataParallel to support fixed some parameters
    if dist_train:
        model = x2ms_nn.DistributedDataParallel(model, device_ids=[cfg.LOCAL_RANK % x2ms_distributed.cuda_device_count()], find_unused_parameters=True)
    logger.info(model)
